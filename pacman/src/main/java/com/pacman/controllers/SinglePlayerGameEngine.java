...
public class SinglePlayerGameEngine extends GameEngine implements Runnable {
	...
	public void updateGame() {
		...
		if (collidableObject != null && collidableObject.isCollidableWith(_pacman)) {
			map[_pacman.getPosition().y][_pacman.getPosition().x].collideWith(_pacman, this,0);
		}
	 ...
	 */
	@Override
	public void eatPill(Pacman pacman, Pill pill, int id) {
	 ...
	 */
	@Override
	public void eatSuperPill(Pacman pacman, SuperPill pill, int id) {
	 ...
	 */
	@Override
	public void eatMightyPill(Pacman pacman, MightyPill pill, int id) {
		removeStationaryObjectFromBoard(pacman, pill);
		_points += POINTS_EATING_SUPER_PILL;
		_remainingPills--;
		SoundPlayer.playEatSuperPillSound();
